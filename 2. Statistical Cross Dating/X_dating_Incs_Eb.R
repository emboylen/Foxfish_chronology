...
# Read in csv files
list.files()
SouthFOX <- read.csv("SouthFoxEB.csv")
SouthFOX <- SouthFOX[,-1] # get rid of Year col
x <- colnames(SouthFOX)
y <- substring(x, 2)
colnames(SouthFOX) <- c("row.names", y[2:length(y)])
write.rwl(rwl.df = SouthFOX, fname = "SouthFOXEB.rwl", format = "tucson")
data <- read.rwl("SouthFOXEB.rwl", format = "tucson")
...
write.csv(total, "Xdating_Output_EB.csv", row.names=F)
