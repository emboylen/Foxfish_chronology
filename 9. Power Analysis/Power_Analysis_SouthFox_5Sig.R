...
#list.files()
Data <- read.csv("Foxfish_Biodata_South.csv")
attach(Data)
...
Alpha <- 0.05 # Specified level of signficance
...
p3 <- ggplot(df, aes(x=SampSizeVec, y=ProbVec)) + geom_line() + theme_classic() +
...
p4 <- ggplot(df, aes(x=SampSizeVec, y=ProbVec)) + geom_line() + theme_classic() +
